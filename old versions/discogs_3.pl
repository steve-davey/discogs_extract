...
my (@releases, $rel_id, $line, $country, $released, @genres, @styles, @tracklist,
...
while (<KOTB>) {
    ...
    if (defined($release)) {
            ...
            print join(",",
                ...
                $released,
                defined($styles[0])
                     ? '"'.$styles[0].'"'         # there is one style
                     :  ( defined($genres[0])           # else try genre
                            ? '"'.$genres[0].'"'
                            : ""),);
                #defined($styles[1])
                #     ? '"'.$styles[1].'"'         # there is a second style
                #     :  ( defined($genres[1])           # else try second genre
                #            ? '"'.$genres[1].'"'
                #            : ""),);
